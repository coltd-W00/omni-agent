...
| 2.3 Task Board Kanban View | 8-column kanban grouped by status, polling 5s when any task Running, empty/loading/error states, no-project fallback | yes | no | no | no | implemented | `npm run test` passed; `taskToCardProps` (9 cases) + `KanbanColumn` (5 cases) + `TaskBoard` (8 cases); `_bmad-output/implementation-artifacts/2-3-task-board-kanban-view.md` |
